...
public class RelationalDataAccessApplication implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		splitUpNames.forEach(name -> log.info(String.format(
				"inserting customer records for %s %s", name[0], name[1])));

		jdbcTemplate.batchUpdate("INSERT INTO customers(first_name, last_name) " +
				"VALUES(?,?)", splitUpNames);

		log.info("Querying for customer record where first_name = 'Josh':");
		...
		jdbcTemplate.query("SELECT id, first_name, last_name FROM customers " +
				...
				stream().forEach(customer -> log.info(customer.toString ()));
